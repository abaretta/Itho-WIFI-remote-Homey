...
String varTemperature = "temperature";
String ClientIP;
...
String navbar       =  "<nav class='navbar navbar-default'><div class='container-fluid'><div class='navbar-header'><a class='navbar-brand' href='/'>Itho control panel</a></div><div><ul class='nav navbar-nav'><li><a href='/'><span class='glyphicon glyphicon-question-sign'></span> Status</a></li><li class='dropdown'><a class='dropdown-toggle' data-toggle='dropdown' href='#'><span class='glyphicon glyphicon-cog'></span> Tools<span class='caret'></span></a><ul class='dropdown-menu'><li><a href='/updatefwm'>Firmware</a></li><li><a href='/api?action=reset&value=true'>Restart</a></ul></li><li><a href='https://github.com/abaretta/Itho-WIFI-remote-Homey' target='_blank'><span class='glyphicon glyphicon-question-sign'></span> Help</a></li></ul></div></div></nav>  ";
...
String ithocontrol     =  "<a href='/button?action=Low'<button type='button' class='btn btn-default'> Low</button></a><a href='/button?action=Medium'<button type='button' class='btn btn-default'> Medium</button></a><a href='/button?action=High'<button type='button' class='btn btn-default'> High</button><a href='/button?action=Timer'<button type='button' class='btn btn-default'> Timer</button></a></a><br><a href='/button?action=Learn'<button type='button' class='btn btn-default'> Learn</button></a><a href='/button?action=State'<button type='button' class='btn btn-default'> Update State</button></a></div>";
...
void setup(void)
{
  ...
  // running WiFi.config after WiFi.begin allows you to only change the IP
  if (FIXEDIP)
  {
...
void loop(void)
{
  ...
  if (timerx10 >= 1 && (millis() - time2) == (timerx10 * 600000))  //every push of timer is 10 minutes
  {
    ...
    eepromWrite(0, 6, "Low");
    handle_homey(CurrentState, varItho);
    handle_root();
  }
...
void handle_DHT() {
  ...
  handle_homey(CurrentHumidity, varHumidity);
  handle_homey(CurrentTemperature, varTemperature);
...
void handle_homey(String data, String variable) {
  String yourdata;
  String URL;
 ...
 // use in combination of Homey builtin webhook (NB: needs to be converted to number using builtin 'calculate variable' action card).
  yourdata = ""+ variable +"?tag="+ data +"";
 // use for HTTP/Better Logic webhook:
 //yourdata = ""+ variable +"/"+ data +"";
 // check content for debugging
  Serial.print(yourdata);
  Serial.print("\r\n");
  Serial.print(variable);
  Serial.print("\r\n");
  
 // Make a flow with an 'HTTP receive POST' trigger card from the 'HTTP request' app in Homey to collect the data,
 // with an HTTP JSONPATH better logic to write the value of the parameter to a Better Logic variable.
 // For controlling the fan HTTP GET action cards can be used, e.g. http://<nodeMcu-hostIP>/button?action=Medium

  URL="GET http://"+ host +"/api/manager/logic/webhook/"+ yourdata +" HTTP/1.1";
  Serial.print(URL);
  Serial.print("\r\n");

 // Make a HTTP request using Homey builtin Logic:
  client.println("GET /api/manager/logic/webhook/"+ yourdata +" HTTP/1.1\r\n");
  client.println();

  Serial.println("Homey updated");
  Serial.print("\r\n");
